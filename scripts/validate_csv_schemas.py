...
SCHEMAS: dict[str, list[str]] = {
    ...
    "recommendation_rationale.csv": ["scenario", "rank", "alternative_id", "alternative", "posture", "score_gap_to_leader", "win_rate", "top3_rate", "evidence_risk_band", "prototype_effort", "hardening_effort", "pilot_operational_rank", "team_rollout_operational_rank", "autonomous_pr_operational_rank", "key_rationale"],
    ...
    "market_maintenance_source_matrix.csv": ["source_id", "title", "year", "evidence_type", "domain", "claim_supported", "confidence", "relevant_reports", "url"],
    "sandbox_deterministic_rankings.csv": ["scenario", "scenario_label", "rank", "sandbox_id", "sandbox", "type", "score"],
    "sandbox_monte_carlo_summary.csv": ["scenario", "sandbox_id", "sandbox", "mean_score", "p10_score", "p90_score", "mean_rank", "win_rate", "top3_rate", "trials"],
    "sandbox_threat_coverage.csv": ["threat_id", "threat", "sandbox_id", "sandbox", "required_controls", "coverage_score", "coverage_band"],
    "sandbox_decision_matrix.csv": ["scenario", "scenario_label", "rank", "sandbox_id", "sandbox", "type", "deterministic_score", "monte_carlo_mean_rank", "win_rate", "top3_rate", "posture", "best_for", "watch_for"],
    "sandbox_source_matrix.csv": ["sandbox_id", "sandbox", "url"],
    ...
    "pilot_decision_scores.example.csv": ["rank", "candidate", "eligible", "final_score", "gate_failures", "task_success_rate", "review_acceptance_rate", "safety_score", "artifact_completeness", "cost_latency_score", "setup_maintenance_score", "market_readiness_score", "reviewer_comprehension_score", "trust_provenance_score", "notes"],
    ...
    "markdown_table_check.csv": ["source_file", "start_line", "rows_checked", "expected_columns", "ok", "message"],
